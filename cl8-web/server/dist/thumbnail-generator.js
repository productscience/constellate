...
function ThumbnailGenerator(admin, fileObject) {
  ...
  function makeThumbnails(origPath) {
    ...
    const smallThumbnailPath = path.join(tmpDir, `thumb_${baseName}-${THUMB_SMALL}${ext}`);
    const largeThumbnailPath = path.join(tmpDir, `thumb_${baseName}-${THUMB_LARGE}${ext}`);
    // Generate a thumbnail using ImageMagick.
    debug('making thumbnails for', tempFilePath);

    return Promise.all([spawn('convert', [origPath, '-thumbnail', `${THUMB_SMALL}>`, smallThumbnailPath]), spawn('convert', [origPath, '-thumbnail', `${THUMB_LARGE}>`, largeThumbnailPath])]).then(results => {
  ...
  function createThumbsForProfile(fetchPath, destPath) {
    ...
    if (!validateObject()) {
      throw new Error("We don't have a valid image object, so we can't make thumbnails from it");
    }
    // begin processing
    return fetchImage(fetchPath, destPath).then(destPath => {
