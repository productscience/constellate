...
function Cl8Importer(importerCredentials) {
   ...
   */
  let importUsersAndTags = (() => {
    var _ref = _asyncToGenerator(function* () {
      const collectedData = yield fetchAllDataforSyncing();

      const enrichedPeeps = buildEnrichedPeeps(collectedData.peeps, collectedData.tags);

      // 2) find ones who don't already exist in the realtime user list already
      const peepsToImport = yield filterOutPeepsToImport(enrichedPeeps, collectedData.fbUserList);

      // 3) import each of the new Peeps:
      // - add them to the Firebase RTB with the airtable ID as the key
      // - add them to Firebase Auth

      const importedPeeps = yield importUsersAcrossServices(peepsToImport, collectedData.fbUserList);
    ...
    return function importUsersAndTags() {
  ...
  let importUsersAcrossServices = (() => {
    var _ref2 = _asyncToGenerator(function* (peepsToImport, userList) {
    ...
    return function importUsersAcrossServices(_x, _x2) {
  ...
  let fetchAllDataforSyncing = (() => {
    var _ref3 = _asyncToGenerator(function* () {
      const tags = yield atbl.getTags();
      const peeps = yield atbl.getUsers();

      // then fetch the user list from Firebase RTB so we can make
      // comparisons about them being in the system
      const fbUserList = yield fbase.getUserList();
    ...
    return function fetchAllDataforSyncing() {
  ...
  const fbase = FireBaseWrapper(importerCredentials.fbaseCreds);function filterOutPeepsToImport(enrichedPeeps, firebaseUserList) {
    function pulloutEmails(list) {
      return _.map(_.values(list), rec => {
        ...
        if (typeof rec !== 'undefined') {
          if (typeof rec.id !== 'undefined' && typeof rec.fields !== 'undefined' && typeof rec.fields.email !== 'undefined') {
